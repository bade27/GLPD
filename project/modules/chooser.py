...
class Chooser(torch.nn.Module):
	...
	def forward(self, embeddings, activity, original):
		...
		available_indices = [pool_of_places.index(place) for place in available_places]
		pool_of_places = available_places

		distribution = self.log_softmax(output[pool_of_places])
		...
		while True:
			...
			if discovered_next != connections:
				for place in pool_of_places:
					if not self.mask[place]:
						self.mask[place] = True
						chosen_places.add(place)
						probability = distribution[pool_of_places.index(place)]
